...
- `@website/shared/ui` — generic mint-theme UI: `Button` (variant + size), `Badge`, `Alert`, `Card`, `Input`/`TextField`/`Textarea`, `Checkbox`, `RadioGroup`, `Switch`, `Select`, `Slider`, `Tabs`, `Accordion`, `Avatar`, `Breadcrumb`, `Pagination`, `Skeleton`, `Spinner`, `Stepper`, `SectionHeader`, `cn`, plus the shadcn `Form`/`Field`/`Label`/`DropdownMenu`/`Dialog` wrappers. Components are imported from the shadcn `new-york-v4` registry and retuned to the mint tokens (focus ring per `DESIGN.md`); Radix primitives come from the single `radix-ui` umbrella package. Live reference for every token and component: **`/design-system`** in the admin app (unlinked route, `admin/src/pages/DesignSystem.tsx`).
